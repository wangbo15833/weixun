...
class User_model extends MY_Model {
     ...
     * 根据店铺ID获取会员列表
     */
    function getUserByShopID($shopid){

        //$sql = 'select u.*,um.nickname as nickname from user as u left join user_material as um on u.id = um.uid  where u.id="'.$id.'"';
        $sql = "select * from mcard as m left join user as u on m.tel= u.tel where shopid=".$shopid;
	...
	function getUserList($data){
		if(isset($data['startdate']))
			$sql="select * from user as u left join mcard as m on u.tel=m.tel left join consumption as c on m.mcid=c.mcid where c.shopid=".$data['shopid']." and c.pubtime>=".$data['startdate']." and c.pubtime<=".$data['enddate']." group by u.id";
		elseif(isset($data['jf']))
			$sql="select * from user as u left join mcard as m on u.tel=m.tel where jf>".$data['jf'];
