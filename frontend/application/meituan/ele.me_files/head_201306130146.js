...
var eleme_validate_necessary = function(b, c, a) {
	$.each(b,
	function() {
		var d = $(this);
		d.live("keyup",
		function() {
			d.parent().parent().removeClass("error");
			d.next("span").attr("hidden", "hidden")
		})
	});
	c.live("click",
	function() {
		...
		eleme.btnInteractive(c);
		a.submit()
	})
};
var eleme_cookie = eleme_cookie || {};
var eleme = eleme || {};
eleme_cookie.domain = "." + eleme.host;
eleme_cookie.suffix = eleme_cookie.domain.replace(/\./g, "_");
eleme_cookie.path = "/";
eleme_cookie.set_cookie = function(b, c, a) {
	if (a != null && a != "") {
		$.cookie(b + "_" + eleme_cookie.suffix, c, {
			expires: a,
			path: eleme_cookie.path,
			domain: eleme_cookie.domain
		})
	} else {
		$.cookie(b + "_" + eleme_cookie.suffix, c, {
			path: eleme_cookie.path,
...
eleme_cookie.get_cookie = function(a) {
	return $.cookie(a + "_" + eleme_cookie.suffix)
};
	/*获取url*/
function get_entry_url() {
	var a = /(\/.+\.php)/.exec(window.location.pathname);
	return a ? a[1] : ""
}
...
function exp_up(e, c) {
	...
	$div.hide();
	a.appendChild(f);
	$div.html('<span class="exp_up">积分+' + e + "</span>");
	$div.css({
		position: "absolute",
		zIndex: 999,
		top: point.top + "px",
	...
	if ($.browser.msie) {
		window.console = {};
		var a = ["log", "error", "warn", "info", "debug", "dir"];
		$.each(a,
		function(b, c) {
			window.console[c] = function() {}
		})
	}
})(); 

(function() {
	var d = ["/../frontend/application/meituan/ele.me_files/home_logo.jpg", "/../frontend/application/meituan/ele.me_files/bg.gif", "/../frontend/application/meituan/ele.me_files/background.jpg"];
	var c = d.length;
	for (var b = 0; b < c; ++b) {
		var a = new Image();
		a.src = eleme.staticHost + d[b]
	}
})();

var login_modal_first = function() {
	if (eleme.logined && $("#modal_globalLogin").size() <= 0) {
		return true
	}
	var a = false;
	if (!a) {
		$("#modal_globalLogin").modal({
			keyboard: true,
			backdrop: "static"
		});
		a = true
	}
	$("#modal_globalLogin").modal("show");
	$("#modal_globalLogin .btn.cancel").click(function() {
		$("#modal_globalLogin").modal("hide")
	});
	return false
};

eleme.btnInteractive = function(a) {
	if (!a.attr("data-loading")) {
		return
	}
	if (a.html()) {
		a.html(a.attr("data-loading"))
	} else {
		a.val(a.attr("data-loading"))
	}
};

var showGlobalLogin = function() {
	$("#modal_globalLogin").modal()
};