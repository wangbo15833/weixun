...
class PGourmet extends CI_Controller {
    const CHANNELID = 2;
    const PAGESIZE = 20;
    ...
    function index(){
        $server = new HessianService(new PGourmet(), array('displayInfo' => true));
        $server -> handle();
    }


    function getTypes(){
        $rel=$this->mtypes->get_types(2,0);
    ...
    function editData(){
        ...
        $data['ddlCategory'] = $this->input->get_post('ddlCategory')? htmlspecialchars($this->input->get_post('ddlCategory')) : '';
        $data['ddlArea'] = $this->input->get_post('ddlArea')? htmlspecialchars($this->input->get_post('ddlArea')) : '';
    ...
    /**
     *添加信息
     * @param title typeId content phone county address map_x map_y price tag pics
     * @return bool
     */
    function doData(){
        $this->load->library('Pinyin');
        $data['name'] = get_post('title') ? htmlspecialchars(get_post('title')) : '';
        $data['type_id'] = get_post('typeId') ? htmlspecialchars(get_post('typeId')) : '';
        ...
        $s = $this->gourmet->addGourmet($data);
        return $s ? true : false;
    }
    ...
    function  doDataPic(){
        ...
        $s = $this->gourmet->addGourmetPic($hid);
        return $s ? true : false;
    ...
    /**
     *添加评论
     * @param c_id commented_id commented_info
     * total_appraisal service_appraisal condition_appraisal uid
     *
     */
