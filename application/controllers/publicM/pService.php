...
class PService extends CI_Controller {
    const CHANNELID = 7;
    const PAGESIZE = 20;
    ...
    function index(){
        $server = new HessianService(new PService(), array('displayInfo' => true));
        $server -> handle();
    }
    ...
    function doSC(){
        ...
        if(! $param['c_cid'] || !$param['c_uid']){ echo json_encode(array('status'=>0));
            return;
        }
