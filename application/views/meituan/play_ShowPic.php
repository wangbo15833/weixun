...
<?php startblock('content') ?>
    ...
    <div class="trans-nav">
        ...
        <a class="btn-upload " title="上传图片" onclick="" href="<?php echo WEB_URL?>play/do_upload?gourmetid=<?php echo $gourmetid; ?>&cid=<?php echo $cid?>" rel="nofollow">上传图片</a>
    </div>
    <!--end of 面包屑-->
...
<?php startblock('foot_js')?>
    <script>
        function changeUrl(type, id){
            ...
            if(type == 0){
                window.location.href=base_url+"play/do_load?gourmetid="+gid;
            }else if(type == 1){
                window.location.href=base_url+"play/do_load?gourmetid="+gid+"&b_type="+id;
            }else{
                window.location.href=base_url+"play/do_load?gourmetid="+gid+"&s_type="+id;
            }
        }
